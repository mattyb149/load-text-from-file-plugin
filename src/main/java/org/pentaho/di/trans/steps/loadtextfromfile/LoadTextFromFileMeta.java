...
{	
  ...
  private boolean addresultfile;
     ...
     */
    public LoadTextFromFileField[] getInputFields()
    {
        return inputFields;
    }
     ...
     */
    public void setInputFields(LoadTextFromFileField[] inputFields)
    {
    ...
    *************************************/
	...
	public Object clone()
	{
		LoadTextFromFileMeta retval = (LoadTextFromFileMeta)super.clone();
		...
		for (int i=0;i<nrFields;i++)
		{
            ...
            {
                retval.inputFields[i] = (LoadTextFromFileField)inputFields[i].clone();
            }
		}		
		return retval;
	}
    ...
    public String getXML()
    {
        ...
        {
            LoadTextFromFileField field = inputFields[i];
	...
	private void readData(Node stepnode) throws KettleXMLException
	{
		try
		{
			...
			for (int i=0;i<nrFields;i++)
			{
				...
				LoadTextFromFileField field = new LoadTextFromFileField(fnode);
				inputFields[i] = field;
			} 
		...
		{
			throw new KettleXMLException(BaseMessages.getString(PKG, "LoadTextFromFileMeta.Exception.ErrorLoadingXML", e.toString()));
		}
	}
	...
	public void allocate(int nrfiles, int nrfields)
	{
		...
		inputFields = new LoadTextFromFileField[nrfields];
	...
	public void setDefault()
	{
	  ...
	  extensionFieldName=null;
		...
		for (int i=0;i<nrFields;i++)
		{
		    inputFields[i] = new LoadTextFromFileField("field"+(i+1));
		}
	...
	throws KettleException
	{
		try
		{
			...
			for (int i=0;i<nrFields;i++)
			{
				...
				field.setElementType( LoadTextFromFileField.getElementTypeByCode( rep.getStepAttributeString (id_step, i, "element_type") ));
				...
				field.setTrimType( LoadTextFromFileField.getTrimTypeByCode( rep.getStepAttributeString (id_step, i, "field_trim_type") ));
		...
		{
			throw new KettleException(BaseMessages.getString(PKG, "LoadTextFromFileMeta.Exception.ErrorReadingRepository"), e);
		}
	}
	...
	throws KettleException
	{
      ...
      rep.saveStepAttribute(id_transformation, id_step, "output_format",   outputFormat);
		...
		{
			throw new KettleException(BaseMessages.getString(PKG, "LoadTextFromFileMeta.Exception.ErrorSavingToRepository", ""+id_step), e);
		}
	}
  ...
  private boolean[] includeSubFolderBoolean()
  {
	...
	public void check(List<CheckResultInterface> remarks, TransMeta transMeta, StepMeta stepMeta, RowMetaInterface prev, String input[], String output[], RowMetaInterface info, VariableSpace space, Repository repository, IMetaStore metaStore)
	{
		...
		if (input.length<=0)
		{		
			cr = new CheckResult(CheckResult.TYPE_RESULT_ERROR, BaseMessages.getString(PKG, "LoadTextFromFileMeta.CheckResult.NoInputExpected"), stepMeta);
			remarks.add(cr);
		}
		else
		{
			cr = new CheckResult(CheckResult.TYPE_RESULT_OK, BaseMessages.getString(PKG, "LoadTextFromFileMeta.CheckResult.NoInput"), stepMeta);
			remarks.add(cr);
		}	
		...
		if(getIsInFields())
		{
			 ...
			 {
				 cr = new CheckResult(CheckResult.TYPE_RESULT_ERROR, BaseMessages.getString(PKG, "LoadTextFromFileMeta.CheckResult.NoField"), stepMeta);
				 remarks.add(cr); 
			 }
			 else
			 {
				 cr = new CheckResult(CheckResult.TYPE_RESULT_OK, BaseMessages.getString(PKG, "LoadTextFromFileMeta.CheckResult.FieldOk"), stepMeta);
				 remarks.add(cr); 
			 }		 
		}
		else
		{
			...
			{
				cr = new CheckResult(CheckResult.TYPE_RESULT_ERROR, BaseMessages.getString(PKG, "LoadTextFromFileMeta.CheckResult.NoFiles"), stepMeta);
				remarks.add(cr);
			}
			else
			{
				cr = new CheckResult(CheckResult.TYPE_RESULT_OK, BaseMessages.getString(PKG, "LoadTextFromFileMeta.CheckResult.FilesOk", ""+fileInputList.getFiles().size()), stepMeta);
				remarks.add(cr);
			}	
		}	
 ...
 public String exportResources(VariableSpace space, Map<String, ResourceDefinition> definitions, ResourceNamingInterface resourceNamingInterface, Repository repository, IMetaStore metaStore) throws KettleException {
	...
	{
		return new LoadTextFromFile(stepMeta, stepDataInterface, cnr, transMeta, trans);
	}

	public StepDataInterface getStepData()
	{
		return new LoadTextFromFileData();
	}
