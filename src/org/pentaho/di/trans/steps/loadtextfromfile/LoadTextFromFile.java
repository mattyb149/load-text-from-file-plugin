   ...
   private void addFileToResultFilesname(FileObject file) throws Exception
   {
	...
	private boolean openNextFile()
	{
		try
		{
			if(meta.getIsInFields())
			{
					...
					if(meta.getIsInFields())
					{
						...
						{	
							...
							if (data.indexOfFilenameField<0)
							{
								...
								logError(BaseMessages.getString(PKG, "LoadTextFromFile.Log.ErrorFindingField")+ "[" + meta.getDynamicFilenameField()+"]");  
								throw new KettleException(BaseMessages.getString(PKG, "LoadTextFromFile.Exception.CouldnotFindField",meta.getDynamicFilenameField()));  
							}
						}
			   ...
			   if(isDetailed()) logDetailed(BaseMessages.getString(PKG, "LoadTextFromFile.Log.Stream", meta.getDynamicFilenameField(),Fieldvalue));
			...
			{
				if (isDetailed()) logDetailed(BaseMessages.getString(PKG, "LoadTextFromFile.Log.OpeningFile", data.file.toString()));
		...
		{
			logError(BaseMessages.getString(PKG, "LoadTextFromFile.Log.UnableToOpenFile", ""+data.filenr, data.file.toString(), e.toString()));
			stopAll();
			setErrors(1);
			return false;
		}
	...
	public boolean processRow(StepMetaInterface smi, StepDataInterface sdi) throws KettleException
	{
		try {
			 ...
			 if (isRowLevel()) logRowlevel(BaseMessages.getString(PKG, "LoadTextFromFile.Log.ReadRow",data.outputRowMeta.getString(outputRowData)));
	...
	private void handleMissingFiles() throws KettleException
	{
		...
		if (nonExistantFiles.size() != 0)
		{
			...
			logError(BaseMessages.getString(PKG, "LoadTextFromFile.Log.RequiredFilesTitle"), 
					BaseMessages.getString(PKG, "LoadTextFromFile.Log.RequiredFiles", message));
		...
		if (nonAccessibleFiles.size() != 0)
		{
			...
			logError(BaseMessages.getString(PKG, "LoadTextFromFile.Log.RequiredFilesTitle"), BaseMessages.getString(PKG, "LoadTextFromFile.Log.RequiredNotAccessibleFiles",message));
			throw new KettleException(BaseMessages.getString(PKG, "LoadTextFromFile.Log.RequiredNotAccessibleFilesMissing",message));
		}
	}
			...
			{	
				...
				switch (loadFileInputField.getElementType())
				{
					case LoadTextFromFileField.ELEMENT_TYPE_FILECONTENT:
						...
						{
							case LoadTextFromFileField.TYPE_TRIM_LEFT  : data.filecontent=Const.ltrim(data.filecontent); break;
							case LoadTextFromFileField.TYPE_TRIM_RIGHT : data.filecontent=Const.rtrim(data.filecontent); break;
							case LoadTextFromFileField.TYPE_TRIM_BOTH  : data.filecontent=Const.trim(data.filecontent); break;
							default: break;
						}
						o=data.filecontent;
					break;
